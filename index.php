...
if (!isset($_SESSION['uid'])) {
	...
	<input type='submit' name='submit' value='Gal'  />
	";
} else {
	echo "welcome ".$_SESSION['username']." &nbsp| <a href='logout_parse.php'>Kabax</a>";
}
?>
...
function topic_replies($cid, $tid) {
	$sql = "SELECT count(*) AS topic_replies FROM posts WHERE category_id='3' AND topic_id='".$tid."'";
	$res = mysql_query($sql) or die(mysql_error());
	$row = mysql_fetch_assoc($res);
...
if (isset($_SESSION['uid'])) {
	$logged = "  <p><a href='create_topic.php?cid=".$cid."'>Halkan Guji- Mawduc samee<p></a>";
} else {
	$logged = " Fadlan Isdiiwan gali Si aad waydiin u samaysid.";
}
// Query that checks to see if the category specified in the $cid variable actually exists in the database
$sql = "SELECT id FROM categories WHERE id='3' LIMIT 1";
// Execute the SELECT query
$res = mysql_query($sql) or die(mysql_error());
// Check if the category exists
if (mysql_num_rows($res) == 1) {
	// Select the topics that are associated with this category id and order by the topic_reply_date
	$sql2 = "SELECT * FROM topics WHERE category_id='3' ORDER BY topic_views DESC LIMIT 10";
	// Execute the SELECT query
	...
	if (mysql_num_rows($res2) > 0) {
		...
		$topics .= "<tr><td colspan='4'><a href='index.php'></a>".$logged."<hr /></td></tr>";
		...
		$when = ($timeAgoObject -> makeAgo($convertedTime)); 
			...
			$topics .= "<tr><td><a href='view_topic.php?cid=".$cid."&tid=".$tid."'>".$title."</a>
			<br /><span class='post_info'>Soo dhigay: ".getusername($creator)." Ilaa ".$when."</span></td>
			<td align='center'>".$last_user."</td><td align='center'>".topic_replies($cid, $tid)."</td>
		...
		echo "<a href='index.php'>Kunoqo Bogga Hore</a><hr />";
